...
Future<void> saveToPath(String path, IkvPackData data, List<Uint8List> values,
  ...
  try {
    if (updateProgress == null || data.originalKeys.length < 100) {
      ...
      var split = 20;
      var chunkSize = (data.originalKeys.length / split).round();
      for (var i = 1; i < split; i++) {
        await insert(db, data.originalKeys, values, chunkSize * (i - 1),
            chunkSize * i - 1);
      ...
      if (!canceled) {
        await insert(db, data.originalKeys, values, chunkSize * (split - 1),
            data.originalKeys.length - 1);
        updateProgress(100);
      }
    }
  } finally {
